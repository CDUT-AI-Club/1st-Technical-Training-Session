...
data_transform = {
    "train": transforms.Compose([transforms.Resize((224, 224)),  # cannot 224, must (224, 224)
                                 transforms.ToTensor(),
                                transforms.RandomHorizontalFlip(),
                                transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])]),
    "val": transforms.Compose([transforms.ToTensor(),
                                transforms.Resize((224, 224)),  # cannot 28, must (28, 28)
                                transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])}
...
# 假设每个图像的像素值是按行展开的，且每个图像的大小为 28x28
# 将 CSV 数据转换为 NumPy 数组




def pre_processing(anwser,data,labels,is_train):
    ...
    # 假设每个图像的像素值范围是 0 到 255
    # 将 NumPy 数组转换为 PIL.Image 对象，并应用预处理
    processed_images = []
    ...
    # 假设标签数据也在 CSV 文件中，或者你可以从其他地方获取
    # 例如，标签数据在另一个 CSV 文件中
    
    labels_tensor = torch.tensor(labels, dtype=torch.long)
    # 使用 TensorDataset 将特征和标签配对
    from torch.utils.data import TensorDataset
    dataset = TensorDataset(features_tensor, labels_tensor)
    # # 重新调整数据形状以满足 AlexNet 的输入要求
    # X_F = torch.stack([data_transform["train"](img) for img in data]).float()  # 提取特征并转换为 Tensor
    # # X_F = data.values.reshape(-1, 784)  # 提取特征（NumPy 数组）
    # Y_F = labels.values                 # 提取标签（NumPy 数组）
    
    # # 转换为 Tensor（注意数据类型）
    # # features_tensor = torch.tensor(X_F, dtype=torch.float32).reshape(-1, 1, 28, 28) / 255.0
    ...
    # dataset = TensorDataset(X_F, labels_tensor)  # 特征和标签配对
    return dataset

training_data=pre_processing("no",X_train,Y_train,is_train=True)

test_data=pre_processing("no",X_val,Y_val,is_train=False)
...
model_name= "vgg16"
net = vgg(model_name, num_classes=10, init_weights=True)
